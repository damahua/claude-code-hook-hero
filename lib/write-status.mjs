...
/**
 * Default model for cost calculation when session model is unknown.
 * Using opus rates to match the dashboard's hardcoded rates.
 */
 ...
 * @param {object} costRates - Cost rates for recalculating cost from tokens
 * @param {Set<string>} activeBufferIds - Session IDs with active buffers (to skip double-counting)
 */
function aggregateCompletedSessions(sessionsDir, status, costRates, activeBufferIds) {
  let files;
  try {
  ...
  for (const file of files) {
    ...
    // Skip sessions that still have an active buffer (dashboard does this too)
    ...
    // Cost — always recalculate from tokens (don't trust baked estimated_cost_usd
    // which is null for sessions with unknown models)
    if (costRates) {
      const model = summary.context?.model || DEFAULT_MODEL;
    ...
    // Interaction time
    status.today.interaction_time_sec += summary.interaction_time_sec ?? 0;

    // Total duration (wall-clock time of completed sessions)
    const timing = summary.timing || {};
    status.today.total_duration_sec += timing.duration_seconds || 0;
  }
}
...
function aggregateActiveBuffers(bufferDir, status, costRates, store) {
  ...
  for (const sessionId of sessionIds) {
    ...
    // Count this as an active session (also counts toward total)
    ...
    // Interaction time
    status.today.interaction_time_sec += buffer.interaction_time_sec ?? 0;

    // Compute duration (wall-clock since session start)
    let durationSec = 0;
    if (buffer.start_time) {
      const startMs = new Date(buffer.start_time).getTime();
      if (!isNaN(startMs)) {
        durationSec = Math.max(0, Math.round((now - startMs) / 1000));
        status.today.total_duration_sec += durationSec;
      }
    }

    // Compute active session cost — fall back to opus rates (matches dashboard)
    let sessionCost = 0;
    if (costRates) {
      const model = buffer.context?.model || DEFAULT_MODEL;
      const cost = calculateCost(model, {
    ...
    status.active.push({
      ...
      project: buffer.context?.project_name || buffer.context?.directory || path.basename(buffer.context?.cwd || '') || null,
  ...
  // Don't round here — rounding happens once after all costs are summed in writeStatus
}
...
export function writeStatus(store, costRates) {
  ...
  // Collect active buffer IDs first — needed to skip double-counting in completed sessions
  const bufferDir = path.join(store.baseDir, 'buffer');
  const activeBufferIds = new Set();
  try {
    for (const f of fs.readdirSync(bufferDir).filter(f => f.endsWith('.buf') || f.endsWith('.json'))) {
      activeBufferIds.add(f.replace(/\.(buf|json)$/, ''));
    }
  } catch {}

  // Aggregate completed sessions (skipping those with active buffers)
  const sessionsDir = path.join(store.baseDir, 'sessions', today);
  aggregateCompletedSessions(sessionsDir, status, rates, activeBufferIds);

  // Aggregate active buffers (adds to today totals, costs, builds active[])
  aggregateActiveBuffers(bufferDir, status, rates, store);

  // Round cost once after all sources are summed
  status.today.cost_usd = Math.round(status.today.cost_usd * 100) / 100;
  for (const entry of status.active) {
    entry.cost_usd = Math.round(entry.cost_usd * 100) / 100;
  }

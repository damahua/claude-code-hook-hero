...
function aggregateCompletedSessions(sessionsDir, status) {
  ...
  for (const file of files) {
    ...
    // Cost — calculateCost() returns null for unknown models, so check for number
...
function aggregateActiveBuffers(bufferDir, status, costRates, store) {
  ...
  for (const sessionId of sessionIds) {
    ...
    // Compute active session cost — fall back to claude-sonnet-4-6 if model unknown
    let sessionCost = 0;
    if (costRates) {
      const model = buffer.context?.model || 'claude-sonnet-4-6';

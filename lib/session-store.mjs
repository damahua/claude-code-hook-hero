...
export class SessionStore {
  ...
  appendEvent(date, sessionId, eventObj) {
    ...
    const frame = this.codec.encodeFrame(eventObj, dict);
    const filePath = this.#eventPath(date, sessionId);
    fs.appendFileSync(filePath, frame);
  }
  ...
  deleteBuffer(sessionId) {
    // Delete both new and legacy buffer files
    for (const ext of [this.codec.bufferExtension, '.json']) {
      try {
  ...
  cleanOrphanedBuffers() {
    ...
    const bufferExts = new Set(['.json', '.buf']);
    const now = Date.now();
